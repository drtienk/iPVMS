...
window.cloudPeriodExchangeRateWriteOnce = async function cloudPeriodExchangeRateWriteOnce(opts) {
  try {
    ...
    if (!periodId || String(periodId).trim() === "") {
      console.warn("[CLOUD][WRITE][PERIOD][EXCHANGE_RATE] periodId not found");
      return { ok: false, reason: "periodId_not_found" };
    }
    ...
    const payloadObj = sheet; // Only the exchange_rate sheet
    let payloadText;
